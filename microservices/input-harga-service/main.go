...
func main() {
	...
	router.HandleFunc("/api/input-harga", HandleInputHarga(connKafka)).Methods(http.MethodPost)

	log.Printf("input-harga-service run at : localhost:%s", servicePort)
	http.ListenAndServe(fmt.Sprintf(":%s", servicePort), router)

}

func HandleInputHarga(connKafka *kafka.Conn) func(w http.ResponseWriter, r *http.Request) {
